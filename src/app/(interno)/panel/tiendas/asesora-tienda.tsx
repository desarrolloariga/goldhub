...
import {
  guardarAsesoraTienda,
  type EstadoAsesora,
} from "@/lib/acciones/tiendas";

/**
 * El asesora de la tienda, editable en la propia fila.
 *
...
export function AsesoraTienda({
  ...
  const [estado, accion, guardando] = useActionState<EstadoAsesora, FormData>(
    guardarAsesoraTienda,
    null,
  );
  const [editando, setEditando] = useState(false);
  const [valor, setValor] = useState(asesora ?? "");
   ...
   */
  const abierto = editando && enviado !== (asesora ?? "");
      ...
      <span className="flex min-w-0 flex-col gap-1">
        ...
        {estado?.error ? (
          ...
          <span className="text-ink/40 text-[11px] leading-snug">
            Quién atiende esta tienda. Déjalo vacío para dejarla sin asignar.
          </span>
        )}
      </span>
    );
  ...
  /*
   * Sin asignar el botón lo dice, y no se queda como un icono mudo: es
   * justamente la tienda a la que hay que ponerle una, así que es cuando más
   * tiene que verse.
   */
  return (
    <button
      ...
      title={
        asesora
          ? `Cambiar el asesora de ${nombre}`
          : `Poner el asesora de ${nombre}`
      }
      aria-label={
        asesora
          ? `Cambiar el asesora de ${nombre}`
